...
### 1. 개요
GrantFollow는 정부지원사업을 수행하는 10인 이하 스타트업 PM/팀장이 사업비 집행 현황을 한눈에 파악하고, 비목(예산 항목)별 지출을 정확히 기록·증빙할 수 있는 정부지원사업비 집행 후속관리 대시보드(MVP)이다.
...
- The five ordered expense stages are `budget_registration` (예산 등록), `pre_approval` (사전 승인), `execution_in_progress` (집행 중), `execution_request` (집행 요청), and `execution_completed` (집행 완료).
