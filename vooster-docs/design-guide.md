...
- 지출 칸반(5단계, 자유 이동)
  ...
  - 단계 이동은 전/후/건너뛰기 모두 허용한다. 드래그 앤 드롭도 모든 다른 단계로 이동할 수 있다.
...
- 지출 상세 풀페이지
  ...
  - 진행상태: 좌측 vertical progress stepper로 사업비 등록 → 사전 승인 → 집행 수행 → 집행 요청 → 집행 완료 5단계를 표시합니다.
  - 본문: 우측 selected-stage workbench에서 선택 단계의 업무절차만 표시합니다. 현재 저장된 단계는 gradient/accent로 더 강하게 강조합니다.
  - 기업양식은 workbench 상단의 닫힌 토글로 제공하고, 현재 지출 비목에 필요한 모든 집행서류를 먼저 보여준 뒤 연결된 기업양식을 각 서류의 하위항목으로 보기/다운로드 제공합니다.
  - 필요 증빙서류는 별도 섹션으로 반복 표시하지 않습니다.
  - 이전 단계 정보는 접힘/펼침이 가능하되, 항상 수정할 수 있어야 합니다.
  - 단계 클릭은 우측 workbench 미리보기만 바꾸며, 실제 저장 단계 변경은 `이 단계로 이동` 버튼으로만 수행합니다.
- 파일 업로드
...
- 지출
  - +지출 → 빠른 등록 페이지 → 카드 생성 → 카드 클릭 → 지출 상세 풀페이지 이동 → 일정/거래처/단계별 업무절차 확인 → 필요한 단계로 이동 → 집행 완료 시 대시보드 자동 반영
## 12. 마이크로카피 가이드
