...
이 문서는 GrantFollow에서 지출이 어떤 단계로 관리되고, 각 단계에서 어떤 정보를 입력해야 하는지 정의하는 도메인 정책 문서다.
...
GrantFollow의 실행 단위는 “비목”이 아니라 “지출”이다.
...
지출 실행 정책은 GrantFollow의 지출을 아래 5단계로 관리하기 위해 정의한다.
...
- 실제 소진액은 집행 신청 단계 기준으로 계산할 수 있게 한다.
```
...
GrantFollow의 지출 단계는 아래 key를 사용한다.
...
GrantFollow에서는 이를 “2천만원 기준 거래”로 표현한다.
...
GrantFollow의 대시보드에서 실제 소진액은 기본적으로 이 단계의 최종 집행 금액을 기준으로 집계한다.
...
GrantFollow의 지출은 단계가 이동할 때마다 필요한 입력값이 점진적으로 추가된다.
...
대시보드의 실제 소진액은 기본적으로 `집행 신청` 단계의 지출만 합산한다.
...
따라서 집행 신청 전 단계의 금액은 운영 현황에는 표시할 수 있지만, 실제 소진액에는 포함하지 않는다.
...
```text
actual_spent_amount = 집행 신청 단계의 최종 집행 금액 합계
```
...
- 대시보드 실제 소진액은 기본적으로 execution_request 단계의 final_amount를 기준으로 계산한다.
...
- 대시보드 실제 소진액은 기본적으로 집행 신청 단계의 최종 집행 금액을 기준으로 계산한다.
