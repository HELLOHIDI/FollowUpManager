...
create index if not exists project_schedules_project_scheduled_on_idx
  on public.project_schedules (project_id, scheduled_on);

create table if not exists public.discord_schedule_reminder_deliveries (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists discord_schedule_reminder_deliveries_status_event_date_idx
  on public.discord_schedule_reminder_deliveries (status, event_date);

create index if not exists discord_schedule_reminder_deliveries_review_idx
...
returns table (id uuid, claim_token uuid, message_content text)
language plpgsql
security definer
...
drop trigger if exists project_schedules_set_updated_at on public.project_schedules;
create trigger project_schedules_set_updated_at
before update on public.project_schedules
for each row execute function public.set_updated_at();

drop trigger if exists discord_schedule_reminder_deliveries_set_updated_at on public.discord_schedule_reminder_deliveries;
...
drop policy if exists "Authenticated users can manage project schedules" on public.project_schedules;
...
drop policy if exists "Authenticated users can read Discord schedule reminder deliveries" on public.discord_schedule_reminder_deliveries;
...
grant select, insert, update, delete on table public.project_schedules to authenticated;
grant select, insert, update, delete on table public.project_schedules to service_role;
grant select, insert, update, delete on table public.discord_schedule_reminder_deliveries to service_role;
