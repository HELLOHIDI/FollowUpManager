...
update public.program_policy_evidence_requirements
set accepted_documents = jsonb_build_array(
  jsonb_build_object(
    'documentKey', coalesce(nullif(document_key, ''), evidence_key),
    'label', evidence_name
  )
)
where accepted_documents is null;

alter table public.program_policy_evidence_requirements
  alter column accepted_documents set default '[]'::jsonb,
  alter column accepted_documents set not null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'program_policy_evidence_requirements_accepted_documents_array'
      and conrelid = 'public.program_policy_evidence_requirements'::regclass
  ) then
    alter table public.program_policy_evidence_requirements
      add constraint program_policy_evidence_requirements_accepted_documents_array
        check (jsonb_typeof(accepted_documents) = 'array');
  end if;
end $$;

create table if not exists public.expense_evidence_requirement_statuses (
  id uuid primary key default gen_random_uuid(),
  project_id uuid not null references public.projects(id) on delete cascade,
  expense_id uuid not null references public.expenses(id) on delete cascade,
  policy_version_id uuid references public.program_policy_versions(id) on delete set null,
  policy_snapshot_hash text not null check (btrim(policy_snapshot_hash) <> ''),
  requirement_key text not null check (requirement_key ~ '^[a-z0-9_]+$'),
  status text not null check (status in ('waived')),
  waived_reason text,
  changed_by uuid references auth.users(id) on delete set null,
  changed_at timestamptz not null default now(),
  constraint expense_evidence_requirement_statuses_unique
    unique (expense_id, policy_snapshot_hash, requirement_key)
);

create index if not exists expense_evidence_requirement_statuses_project_idx
  on public.expense_evidence_requirement_statuses(project_id, expense_id);

alter table public.expense_evidence_requirement_statuses enable row level security;
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'expense_evidence_requirement_statuses'
      and policyname = 'Authenticated users can manage expense evidence requirement statuses'
  ) then
    create policy "Authenticated users can manage expense evidence requirement statuses"
    on public.expense_evidence_requirement_statuses
    for all
    to authenticated
    using (true)
    with check (true);
...
begin
  ...
  for item in select value from jsonb_array_elements(coalesce(p_evidence_requirements, '[]'::jsonb)) loop
    ...
    insert into public.program_policy_evidence_requirements (
      ...
      case
        when jsonb_typeof(item -> 'acceptedDocuments') = 'array' and jsonb_array_length(item -> 'acceptedDocuments') > 0
          then item -> 'acceptedDocuments'
        else jsonb_build_array(
          jsonb_build_object(
            'documentKey', coalesce(nullif(item ->> 'documentKey', ''), item ->> 'evidenceKey'),
  ...
  if locked_project.confirmed_policy_version_id is not null then
    ...
    select jsonb_build_object(
      ...
      'evidence_requirements', coalesce(
        jsonb_agg(
          jsonb_build_object(
            'accepted_documents', coalesce(evidence.accepted_documents, '[]'::jsonb),
