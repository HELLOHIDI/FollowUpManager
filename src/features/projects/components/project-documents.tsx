...
export function ProjectDocuments({
  ...
  return <section className={embedded ? "space-y-4" : "space-y-4 border-t pt-6"}>
    {!embedded ? <div><h2 className="text-lg font-semibold">기관 제공 서류</h2><p className="text-sm text-muted-foreground">등록 후에도 추가하거나 삭제할 수 있습니다.</p></div> : null}
    <label className="inline-flex cursor-pointer items-center gap-2 rounded-md border px-3 py-2 text-sm font-medium"><Upload className="size-4" />파일 추가<Input className="sr-only" multiple type="file" accept=".pdf,.doc,.docx,.hwp,.hwpx,.xls,.xlsx,.csv,.jpg,.jpeg,.png,.webp,.zip" onChange={async (event) => { for (const file of Array.from(event.target.files ?? [])) { try { await uploadMutation.mutateAsync({ file, projectId }); } catch (error) { toast({ title: "파일을 추가하지 못했습니다.", description: extractApiErrorMessage(error), variant: "destructive" }); } } event.target.value = ""; }} /></label>
