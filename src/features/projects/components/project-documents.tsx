...
export function ProjectDocuments({
  ...
  return <section className={embedded ? "space-y-4" : "space-y-4 border-t pt-6"}>
    ...
    <label className="inline-flex cursor-pointer items-center gap-2 rounded-md border px-3 py-2 text-sm font-medium"><Upload className="size-4" />파일 추가<Input className="sr-only" multiple type="file" accept=".pdf,.doc,.docx,.hwp,.hwpx,.xls,.xlsx,.csv,.jpg,.jpeg,.png,.webp,.zip" onChange={async (event) => { for (const file of Array.from(event.target.files ?? [])) { try { await uploadMutation.mutateAsync({ file, projectId, purpose }); } catch (error) { toast({ title: "파일을 추가하지 못했습니다.", description: extractApiErrorMessage(error), variant: "destructive" }); } } event.target.value = ""; }} /></label>
