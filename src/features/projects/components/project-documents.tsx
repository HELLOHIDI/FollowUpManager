...
export function ProjectDocuments({
  ...
  const { deleteDocumentMutation, uploadDocumentsMutation } = useProjectMutations();
  ...
  return <section className={embedded ? "space-y-4" : "space-y-4 border-t pt-6"}>
    ...
    <label className="inline-flex cursor-pointer items-center gap-2 rounded-md border px-3 py-2 text-sm font-medium">
      {uploadDocumentsMutation.isPending ? <Loader2 className="size-4 animate-spin" /> : <Upload className="size-4" />}
      {uploadDocumentsMutation.isPending ? "업로드 중" : "파일 추가"}
      <Input className="sr-only" multiple type="file" accept={DEFAULT_UPLOAD_ACCEPT} onChange={async (event) => {
        const files = Array.from(event.target.files ?? []);
        if (files.length === 0) return;
        try {
          const result = await uploadDocumentsMutation.mutateAsync({ files, projectId, purpose });
          if (result.failed > 0) toast({ title: "일부 파일을 추가하지 못했습니다.", description: `${result.succeeded}개 성공, ${result.failed}개 실패`, variant: "destructive" });
        } catch (error) {
