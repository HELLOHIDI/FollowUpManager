...
export function ProjectForm({ assignmentError, companyName, initialValues = EMPTY_PROJECT, isSubmitting, onDirtyChange, onSubmit, projects = [], showAttachments = false, submitLabel = "사업 등록" }: {
  ...
  return <form className="grid gap-5 sm:grid-cols-2" onSubmit={form.handleSubmit((input) => onSubmit(input, files))}>
    ...
    {showAttachments ? <label className="grid gap-2 text-sm font-medium sm:col-span-2">기관 제공 서류<Input accept=".pdf,.doc,.docx,.hwp,.hwpx,.xls,.xlsx,.csv,.jpg,.jpeg,.png,.webp,.zip" multiple type="file" onChange={(event) => setFiles(Array.from(event.target.files ?? []))} /><span className="text-xs text-muted-foreground">파일당 최대 20MB, 여러 파일을 선택할 수 있습니다.</span></label> : null}
