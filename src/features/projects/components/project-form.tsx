...
export function ProjectForm({ assignmentError, companyName, initialValues = EMPTY_PROJECT, isSubmitting, onDirtyChange, onSubmit, projects = [], showAttachments = false, submitLabel = "사업 등록" }: {
  ...
  return <form className="grid gap-5 sm:grid-cols-2" onSubmit={form.handleSubmit((input) => onSubmit(input, files))}>
    ...
    <label className="grid gap-2 text-sm font-medium">정부지원금 비율(%)<Input inputMode="decimal" {...form.register("governmentSubsidyRatio", { deps: ["selfCashRatio", "selfInKindRatio"] })} /><ErrorText message={form.formState.errors.governmentSubsidyRatio?.message} /></label>
    <label className="grid gap-2 text-sm font-medium">현금 비율(%)<Input inputMode="decimal" {...form.register("selfCashRatio", { deps: ["governmentSubsidyRatio", "selfInKindRatio"] })} /><ErrorText message={form.formState.errors.selfCashRatio?.message} /></label>
    <label className="grid gap-2 text-sm font-medium">현물 비율(%)<Input inputMode="decimal" {...form.register("selfInKindRatio", { deps: ["governmentSubsidyRatio", "selfCashRatio"] })} /><ErrorText message={form.formState.errors.selfInKindRatio?.message} /></label>
