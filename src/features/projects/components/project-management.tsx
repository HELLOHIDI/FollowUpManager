...
export function ProjectManagement({ projectId }: { projectId: string }) {
  ...
  useDirtyNavigationGuard(projectDirty || templateDirty);
    ...
    <>
          ...
          <ProjectForm
            ...
            onSubmit={async (input) => {
              try {
                ...
                const updated = await updateMutation.mutateAsync({ input, projectId });
                setProjectDirty(false);
                toast({ title: "사업 정보를 수정했습니다.", description: `${updated.projectName} 정보를 저장했습니다.` });
              } catch (error) {
                if (extractApiErrorCode(error) === "PROJECT_ASSIGNMENT_NUMBER_CONFLICT") {
                  setAssignmentError("같은 기업에 이미 등록된 과제번호입니다.");
                }
                toast({ title: "사업 정보를 수정하지 못했습니다.", description: extractApiErrorMessage(error), variant: "destructive" });
              }
            }}
      ...
      <div className="mt-6">
        <Card className="shadow-none">
          ...
          <CardContent>
            <ProjectTemplateLinking projectId={projectId} onDirtyChange={setTemplateDirty} />
          </CardContent>
        </Card>
      </div>
