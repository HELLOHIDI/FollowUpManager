...
import { createProjectRequest, deleteProjectDocumentRequest, fetchCompanyProjects, fetchProject, fetchProjectDocuments, updateProjectRequest, uploadProjectDocument } from "../api";
...
export const useProjectMutations = () => {
  ...
  const deleteDocumentMutation = useMutation({ mutationFn: deleteProjectDocumentRequest, onSuccess: (_, variables) => void queryClient.invalidateQueries({ queryKey: projectKeys.documents(variables.projectId) }) });
  return { createMutation, deleteDocumentMutation, updateMutation, uploadMutation };
};
