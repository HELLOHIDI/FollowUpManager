...
export const expenseStageDetailCopy: Record<ExpenseStageKey, { description: string; fields: ExpenseStageMemoFieldKey[] }> = {
  ...
  pre_approval: {
    description: "사전 승인 상태와 승인 참고 정보를 관리합니다.",
    fields: ["approvalReference", "preApprovalMemo"],
  },
  execution_in_progress: {
    description: "집행 수행 중 필요한 진행 메모와 산출물 메모를 관리합니다.",
    fields: ["executionMemo", "deliverableMemo"],
  },
  execution_request: {
    description: "집행 요청 상태, 요청일, 최종 요청 메모를 관리합니다.",
    fields: ["executionRequestMemo"],
  },
  execution_completed: {
    description: "집행 완료 상태와 최종 증빙 상태를 확인합니다.",
    fields: [],
  },
};
...
export const executionProgressStatuses = [
  ["not_started", "수행 전"],
  ["in_progress", "수행 중"],
  ["delayed", "지연"],
  ["completed", "수행 완료"],
...
export const executionRequestStatuses = [
  ["draft", "작성 중"],
  ["ready_to_submit", "요청 준비 완료"],
  ["submitted", "요청 완료"],
